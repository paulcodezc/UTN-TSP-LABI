...
{
    LinkedList* this; //Crea un puntero de tipo LinkedList
    this = (LinkedList*)malloc(sizeof(LinkedList)); //Reservamos memoria dinamica
    if(this != NULL) //Comprobamos que se haya reservado correctamente
    {
        this->pFirstNode = NULL; //Inicializamos los campos del LinkedList
        this->size = 0; //Inicializamos los campos del LinkedList
    }
    return this; //Devuelve la direccion de memoria del puntero con los datos inicializados
}
...
{
    int returnAux = -1; //Una variable para retornas con una respuesta negativa

    if(this != NULL) //Comprueba que el puntero pasado por parametro NO sea NULL
    {
        returnAux = this->size; //A la variable que retorna le asigna pisando el valor negativo con el tamaño del campo "size"
    }
    return returnAux; //Devuelve [-1] en caso de que el puntero sea NULL o devuelve el tamaño o cantidad de elementos de la lista
}
    ...
    if(this != NULL && pFunc != NULL && (order == 1 || !order) && this->size > 0)
    {
        ...
        do
        {
            ...
            if(pNode != NULL)
            {
                ...
                for(i = 0 ; i <(len-1) ; i++)
                {
                    if((pFunc(pNode->pElement, pNextNode->pElement) == 1 && order) || (pFunc(pNode->pElement, pNextNode->pElement) == -1 && !order))
                    {
                        aux = pNode->pElement;
